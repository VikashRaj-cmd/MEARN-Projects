...
export const generateFlashcards = async (text, count = 10) => {
    ...
    try {
        ...
        //Parse the response
        const flashcards = [];
        const cards = generatedText.trim().split('---').filter(c => c.trim());
...
export const generateSummary = async (text) => {
    const prompt = `Provide a concise summary of the following text, highlighting the key points, main ideas, and important points.
...
export const chatWithDocument = async (question, chunks) => {
    ...
    try {
        ...
        console.error('Gemini API error:', error);
        throw new Error('Failed to chat request');
    }
};
